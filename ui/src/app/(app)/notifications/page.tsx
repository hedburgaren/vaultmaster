...
export default function NotificationsPage() {
  ...
  const typeLabel: Record<string, string> = { slack: 'Slack', ntfy: 'ntfy', telegram: 'Telegram', email: 'E-post', webhook: 'Webhook' };

  return (
    <div>
      <div className="flex items-start justify-between mb-6">
        <div>
          <h1 className="text-[28px] font-bold text-vm-text-bright tracking-wide uppercase">Notifikationer</h1>
          <div className="font-mono text-xs text-vm-accent tracking-[2px] mt-1">// NOTIFICATION CHANNELS · {channels.length} ST</div>
        </div>
        <button onClick={() => setShowForm(!showForm)} className="flex items-center gap-2 px-5 py-2.5 bg-vm-accent text-vm-bg rounded font-bold text-sm tracking-wider uppercase hover:bg-[#33ddff] transition-all glow">
          <Plus className="w-4 h-4" /> Ny kanal
        </button>
      </div>
      ...
      {showForm && (
        <div className="bg-vm-surface border border-vm-border-bright rounded p-6 mb-6">
          <div className="grid grid-cols-2 gap-4 mb-4">
            ...
            <div>
              <label className="block font-mono text-[11px] text-vm-text-dim tracking-[2px] uppercase mb-2">Triggers (komma-sep)</label>
      ...
      <div className="space-y-3">
        {channels.map((c: any) => (
          <div key={c.id} className="bg-vm-surface border border-vm-border rounded p-5 flex items-center gap-4 hover:border-vm-border-bright transition-all">
            ...
            <div className="flex-1">
              ...
              <div className="font-mono text-[11px] text-vm-text-dim">{typeLabel[c.channel_type] || c.channel_type} · {c.triggers?.join(', ') || 'Inga triggers'}</div>
            </div>
            {testResults[c.id] && (
            ...
            <div className="flex gap-2">
              ...
              <button onClick={async () => { if (confirm('Ta bort?')) { await deleteNotificationChannel(c.id); load(); }}} className="flex items-center gap-1 px-3 py-1.5 border border-vm-danger text-vm-danger rounded text-xs font-bold tracking-wider uppercase hover:bg-vm-danger/10">
        ...
        {channels.length === 0 && (
          <div className="text-center py-12 text-vm-text-dim font-mono">
            ...
            <div className="tracking-[2px]">Inga notifikationskanaler konfigurerade</div>
          </div>
        )}
      </div>
    </div>
