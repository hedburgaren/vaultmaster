...
export default function DashboardPage() {
  ...
  const backupAgeLabel = data.hours_since_last_backup === null ? 'No backups yet' : data.hours_since_last_backup < 1 ? 'Less than 1h ago' : `${Math.round(data.hours_since_last_backup)}h ago`;
    ...
    <div>
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <div className="font-mono text-xs text-vm-accent tracking-[2px] mt-1">// BACKUP CONTROL CENTER · v2.0</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        ...
        <StatCard label="Failed (24h)" value={data.runs_failed_24h} sub={data.runs_failed_24h > 0 ? 'Check logs' : 'All clear'} color={data.runs_failed_24h > 0 ? 'danger' : 'green'} delay={150} />
      </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        ...
        <div className="bg-vm-surface border border-vm-border rounded p-5 hover:border-vm-border-bright transition-all">
          <div className="flex items-center gap-2 mb-3">
            ...
            <div className="font-mono text-[11px] text-vm-text-dim tracking-[2px] uppercase">Last Successful Backup</div>
          </div>
        ...
        <div className="bg-vm-surface border border-vm-border rounded p-5 hover:border-vm-border-bright transition-all">
          <div className="flex items-center gap-2 mb-3">
            ...
            <div className="font-mono text-[11px] text-vm-text-dim tracking-[2px] uppercase">Backup Vault</div>
          </div>
          <div className="font-code text-2xl font-bold text-vm-accent">{data.total_artifacts}</div>
          <div className="font-mono text-[10px] text-vm-text-dim mt-1">{formatBytes(data.total_artifact_bytes)} total</div>
        </div>
        ...
        <div className="bg-vm-surface border border-vm-border rounded p-5 hover:border-vm-border-bright transition-all">
          <div className="flex items-center gap-2 mb-3">
            ...
            <div className="font-mono text-[11px] text-vm-text-dim tracking-[2px] uppercase">System Health</div>
          </div>
          {(() => {
            const issues: string[] = [];
            if (data.runs_failed_24h > 0) issues.push(`${data.runs_failed_24h} failed backup${data.runs_failed_24h > 1 ? 's' : ''}`);
            if (data.storage_warnings?.length > 0) issues.push(`${data.storage_warnings.length} storage warning${data.storage_warnings.length > 1 ? 's' : ''}`);
            const offlineServers = data.server_health?.filter((s: any) => !s.online).length || 0;
            if (offlineServers > 0) issues.push(`${offlineServers} server${offlineServers > 1 ? 's' : ''} offline`);
            if (data.hours_since_last_backup !== null && data.hours_since_last_backup > 48) issues.push('No backup in 48h+');
            if (issues.length === 0) {
              return (
                <div>
                  <div className="font-code text-2xl font-bold text-vm-success">HEALTHY</div>
                  <div className="font-mono text-[10px] text-vm-text-dim mt-1">All systems operational</div>
                </div>
              );
            }
            return (
              <div>
                <div className="font-code text-2xl font-bold text-vm-danger">{issues.length} ISSUE{issues.length > 1 ? 'S' : ''}</div>
      ...
      {data.storage_warnings?.length > 0 && (
        <div className="mb-6">
          <div className="flex items-center gap-2.5 mb-4">
            ...
            <h2 className="text-base font-bold text-vm-text-bright tracking-[2px] uppercase">Storage Warnings</h2>
          </div>
      ...
      {data.server_health?.length > 0 && (
        <div className="mb-6">
          <div className="flex items-center gap-2.5 mb-4">
            ...
            <h2 className="text-base font-bold text-vm-text-bright tracking-[2px] uppercase">Server Health</h2>
          </div>
            ...
            {data.server_health.map((s: any) => (
              <div key={s.id} className={`bg-vm-surface border rounded p-4 hover:border-vm-border-bright transition-all ${s.online ? 'border-vm-border' : 'border-vm-danger/40'}`}>
                ...
                <div className="font-mono text-[10px] text-vm-text-dim mt-1">
                  {s.online ? (
                    <span className="text-vm-success">Online</span>
                  ) : (
                    <span className="text-vm-danger">Offline{s.last_seen_hours_ago !== null ? ` · last seen ${Math.round(s.last_seen_hours_ago)}h ago` : ''}</span>
                  )}
                </div>
                {s.tags?.length > 0 && (
