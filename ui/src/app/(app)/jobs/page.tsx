...
export default function JobsPage() {
  ...
  const handleDelete = async (id: string) => {
    if (confirm('Ta bort jobb?')) { await deleteJob(id); load(); }
  };
    ...
    <div>
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <div className="font-mono text-xs text-vm-accent tracking-[2px] mt-1">// SCHEMALAGDA JOBB · {jobs.length} ST</div>
        </div>
      ...
      {showForm && (
        <div className="bg-vm-surface border border-vm-border-bright rounded p-6 mb-6">
          <h3 className="text-lg font-bold text-vm-text-bright mb-4 uppercase tracking-wider">Nytt backup-jobb</h3>
          <div className="grid grid-cols-2 gap-4 mb-4">
            ...
            <div>
              <label className="block font-mono text-[11px] text-vm-text-dim tracking-[2px] uppercase mb-2">Typ</label>
            ...
            <div>
              <label className="block font-mono text-[11px] text-vm-text-dim tracking-[2px] uppercase mb-2">Schema (cron)</label>
            ...
            <div>
              <label className="block font-mono text-[11px] text-vm-text-dim tracking-[2px] uppercase mb-2">Domän</label>
            ...
            <div>
              <label className="block font-mono text-[11px] text-vm-text-dim tracking-[2px] uppercase mb-2">Taggar</label>
      ...
      <div className="bg-vm-surface border border-vm-border rounded overflow-hidden">
        <table className="w-full">
          <thead>
            <tr className="bg-vm-surface2 border-b border-vm-border">
              <th className="px-4 py-3 text-left font-mono text-[11px] text-vm-text-dim tracking-[2px] uppercase font-normal">Typ</th>
              <th className="px-4 py-3 text-left font-mono text-[11px] text-vm-text-dim tracking-[2px] uppercase font-normal">Namn</th>
              <th className="px-4 py-3 text-left font-mono text-[11px] text-vm-text-dim tracking-[2px] uppercase font-normal">Schema</th>
              <th className="px-4 py-3 text-left font-mono text-[11px] text-vm-text-dim tracking-[2px] uppercase font-normal">Domän</th>
          ...
          <tbody>
            {jobs.map((j: any) => (
              <tr key={j.id} className="border-b border-vm-border/50 hover:bg-vm-surface2 transition-colors">
                ...
                <td className="px-4 py-3"><Badge status={j.is_active ? 'success' : 'cancelled'} label={j.is_active ? 'AKTIV' : 'INAKTIV'} /></td>
                <td className="px-4 py-3">
                  <div className="flex gap-2">
                    <button onClick={() => handleTrigger(j.id)} className="flex items-center gap-1 px-3 py-1.5 bg-vm-accent text-vm-bg rounded text-xs font-bold tracking-wider uppercase"><Play className="w-3 h-3" /> Kör</button>
        ...
        {jobs.length === 0 && (
          <div className="text-center py-12 text-vm-text-dim font-mono tracking-[2px]">Inga backup-jobb konfigurerade</div>
        )}
      </div>
    </div>
  );
