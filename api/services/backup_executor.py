...
async def execute_postgresql_backup(server, job, run_id: str, db=None) -> dict:
    ...
    try:
        db_name = _safe_ident(config.get("db_name", "postgres"), "db_name")
        pg_user = _safe_ident(config.get("pg_user", "postgres"), "pg_user")
    ...
    try:
        ...
        if dump_format == "custom":
            dump_cmd = f"pg_dump -U {pg_user_q} -Fc -Z {compress_level} {db_name_q} > {remote_path_q}"
        else:
            dump_cmd = f"pg_dump -U {pg_user_q} {db_name_q} | gzip -{compress_level} > {remote_path_q}"
